	...
	*/
	...
	$.fn.loadGrid = function(user_opts) {
		return this.each(function() {
			...
			var opts = $.extend({
				...
				loadingMessage:"Chargement veuillez patienter...",// if blockUI is available, this is the message it will use
			...
			if($grid.attr("title") && !$grid.parents(".gridContainer").find(".gridTitle").length) {
				...
				// add the add button if its on and the title bar is there
					if($grid.data().adding) {
						$grid.parents(".gridContainer").find(".gridTitle").append("<div class='gridButton gridAdd'><div>Ajouter</div></div>");
					...
					// add button if its being routed	
						...
						$grid.parents(".gridContainer").find(".gridTitle").append("<a href='"+$grid.data().addButton+"' class='gridButton gridAdd'><div>Ajouter</div></a>");
					}
			...
			$.post($grid.attr("action"),$grid.data(), function(data, status, xhr) {
				...
				if(data.rows && data.rows.toString() == "[object Object]") {
					...
					$grid.append("<tr><td colspan=100>Pas de colonne</td></tr>");
				}
	...
	$(".gridHeaderRow th").live("click",function(e) {
		...
		if($grid.find("tr.toBeSaved").length && $grid.data().confirmBeforeSort) {
			cont = confirm("Vous n'avez pas enregistrer les changements,ils vont etre definitivement perdus. Continuer?");
		}
	...
	$(".gridHeaderRow th").live("contextmenu",function(e) {
		...
		$(this).getTdsFromTh().each(function(i) {
			...
			if(val.search(/^[\d.\$ۣ��]*$/) == -1) foundLetter = true;
			// parseFloat for calculations
			var floatVal = parseFloat(val.replace(/[\$ۣ�]/,''));
			sum += floatVal;
		...
		var $context = $("\
			<div class='gridContext'>\
				...
				<div class='makeFluid'>Fluide</div>\
				<div class='colHide'>Retirer</div>\
				<div class='colHighlite'>eclairer</div>\
				<div class='colDeHighlight'>obscurcir</div>\
				...
				<div style='display:"+checkboxDisplay+"' class='checkAll'>selectionner tout</div>\
				<div style='display:"+checkboxDisplay+"' class='uncheckAll'>deselectionner tout</div>\
			</div>\
		").css({
	...
	$.fn.setupInlineEditing = function() {
		...
		var saveBtnText = "<div class='gridButton gridSave'><div>Enregistrer</div></div>"; 
		// remvoe all of them
	...
	$.fn.createDeleteColumn = function() {
		...
		$grid.find("tr").each(function(i) {
			var $tr = $(this);
			$del = $("<div class='gridButton gridDelete'><div>Effacer</div></div>");
			$del.click(function() {
				...
				if($grid.data().deleteConfirm) {
					...
					if(confirm("Vous etes sur de vouloir effacer"+whatToSay+" ?")) {
						$.post($grid.attr("action"),{
							"delete":true,
							...
							if(success) {
								...
								alert("Erreur: effacement echoué");
							}
						});
					}	
				} else {
		...
		if(!$pager.length) {
			...
			if($grid.data().dateRange) {
				dateRange = "\
					...
					<div class='gridButton dateGo'><div>Recherche</div></div>\
				";
			}
			
			var refreshButton = "";
			if($grid.data().refreshButton) {
				refreshButton = "<div class='gridButton gridRefresh'><div>Rafraichir</div></div>";
			}
			...
			$pager = $("\
				<div class='gridPager'>\
					...
					<div class='gridButton gridBack'><div>Retour</div></div>\
					<div class='gridButton gridNext'><div>Suivant</div></div>\
					<div class='gridTotal'></div>\
					"+dateRange+"\
					<div class='gridButton gridSave'><div>Enregistrer</div></div>\
				</div>\
			");
